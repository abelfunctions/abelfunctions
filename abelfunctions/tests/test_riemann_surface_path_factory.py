...
class TestConstruction(unittest.TestCase):
    ...
    def test_base_point_and_sheets(self):
        ...
        self.assertAlmostEqual(sheets[0], -1.0j)
        self.assertAlmostEqual(sheets[1], 1.0j)
    ...
    def test_base_point_and_sheets_given(self):
        ...
        self.assertAlmostEqual(PF.base_point, -2)
        sheets = PF.base_sheets
        self.assertAlmostEqual(sheets[0], -1.0j * sqrt(2))
        self.assertAlmostEqual(sheets[1], 1.0j * sqrt(2))

        PF = RiemannSurfacePathFactory(self.X1, base_sheets=[1.0j, -1.0j])
        self.assertAlmostEqual(PF.base_point, -1)
        sheets = PF.base_sheets
        self.assertAlmostEqual(sheets[0], 1.0j)
        self.assertAlmostEqual(sheets[1], -1.0j)
    ...
    def test_incorrect_sheets_above_base_point(self):
        with self.assertRaises(ValueError):
            RiemannSurfacePathFactory(self.X1, base_point=-1, base_sheets=[42, 101])
...
class TestFromComplexPath(unittest.TestCase):
    ...
    def test_primitive_line_smale(self):
        PF = RiemannSurfacePathFactory(
            self.X1, base_point=-1, base_sheets=[-1.0j, 1.0j]
        )
        gamma_x = ComplexLine(-1, -2)
        gamma = PF.RiemannSurfacePath_from_complex_path(gamma_x)
        self.assertAlmostEqual(gamma.get_x(1.0), -2)
        self.assertAlmostEqual(gamma.get_y(1.0)[0], -1.0j * sqrt(2))
        self.assertAlmostEqual(gamma.get_y(1.0)[1], 1.0j * sqrt(2))
    ...
    def test_primitive_line_puiseux(self):
        ...
        self.assertAlmostEqual(gamma.get_y(0.5)[0], -1.0j * sqrt(0.5))
        self.assertAlmostEqual(gamma.get_y(0.5)[1], 1.0j * sqrt(0.5))
    ...
    def test_composite_line_smale(self):
        PF = RiemannSurfacePathFactory(
            self.X1, base_point=-3, base_sheets=[-1.0j * sqrt(3), 1.0j * sqrt(3)]
        )
        gamma_x = ComplexLine(-3, -2) + ComplexLine(-2, -1)
        gamma = PF.RiemannSurfacePath_from_complex_path(gamma_x)
        self.assertAlmostEqual(gamma.get_x(0.0), -3)
        self.assertAlmostEqual(gamma.get_y(0.0)[0], -1.0j * sqrt(3))
        self.assertAlmostEqual(gamma.get_y(0.0)[1], 1.0j * sqrt(3))

        self.assertAlmostEqual(gamma.get_x(0.5), -2)
        self.assertAlmostEqual(gamma.get_y(0.5)[0], -1.0j * sqrt(2))
        self.assertAlmostEqual(gamma.get_y(0.5)[1], 1.0j * sqrt(2))

        self.assertAlmostEqual(gamma.get_x(1.0), -1)
        self.assertAlmostEqual(gamma.get_y(1.0)[0], -1.0j)
        self.assertAlmostEqual(gamma.get_y(1.0)[1], 1.0j)
    ...
    def test_composite_line_mixed(self):
        PF = RiemannSurfacePathFactory(
            self.X1, base_point=-2, base_sheets=[-1.0j * sqrt(2), 1.0j * sqrt(2)]
        )
        gamma_x = ComplexLine(-2, -1) + ComplexLine(-1, 0)
        gamma = PF.RiemannSurfacePath_from_complex_path(gamma_x)
        self.assertEqual(len(gamma.segments), 2)
        self.assertTrue(isinstance(gamma.segments[0], RiemannSurfacePathSmale))
        self.assertTrue(isinstance(gamma.segments[1], RiemannSurfacePathPuiseux))

        self.assertAlmostEqual(gamma.get_x(0.0), -2)
        self.assertAlmostEqual(gamma.get_y(0.0)[0], -1.0j * sqrt(2))
        self.assertAlmostEqual(gamma.get_y(0.0)[1], 1.0j * sqrt(2))

        self.assertAlmostEqual(gamma.get_x(0.5), -1)
        self.assertAlmostEqual(gamma.get_y(0.5)[0], -1.0j)
        self.assertAlmostEqual(gamma.get_y(0.5)[1], 1.0j)
...
class TestMonodromyPath(unittest.TestCase):
    ...
    def test_monodromy_path(self):
        ...
        PF = RiemannSurfacePathFactory(
            self.X2, base_point=-2, base_sheets=[-sqrt(5), sqrt(5)]
        )
    ...
    def test_monodromy(self):
        ...
        PF = RiemannSurfacePathFactory(self.X2)
        branch_points, permutations = PF.monodromy_group()
        self.assertCountEqual(branch_points, [QQbar(z) for z in [-I, I]])
        self.assertEqual(permutations[0], Permutation([1, 0]))
        self.assertEqual(permutations[1], Permutation([1, 0]))

        PF = RiemannSurfacePathFactory(self.X3)
        branch_points, permutations = PF.monodromy_group()
        self.assertCountEqual(branch_points, [QQbar(z) for z in [-I, -1, 1, I]])
        self.assertEqual(permutations[0], Permutation([1, 0]))
        self.assertEqual(permutations[1], Permutation([1, 0]))
        ...
        branch_points, permutations = PF.monodromy_group()
        self.assertCountEqual(branch_points, [QQbar(z) for z in [-I, I]] + [oo])
...
class TestCycles(unittest.TestCase):
    ...
    def test_f2_from_cycle_issue(self):
        ...
        for cycle in a_tuples + b_tuples:
            PF.RiemannSurfacePath_from_cycle(cycle)
...
class TestGenus43(unittest.TestCase):
    ...
    def test_monodromy_path_construction(self):
        ...
        X.path_factory.monodromy_path(b[2])
        X.path_factory.monodromy_path(b[5])
